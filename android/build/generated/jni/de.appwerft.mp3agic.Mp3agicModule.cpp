...
Local<FunctionTemplate> Mp3agicModule::getProxyTemplate(v8::Isolate* isolate)
{
	...
	// Method bindings --------------------------------------------------------
	titanium::SetProtoMethod(isolate, t, "getExampleProp", Mp3agicModule::getExampleProp);
	titanium::SetProtoMethod(isolate, t, "setExampleProp", Mp3agicModule::setExampleProp);
	titanium::SetProtoMethod(isolate, t, "example", Mp3agicModule::example);
	...
	instanceTemplate->SetAccessor(
		...
		Mp3agicModule::getter_exampleProp,
		Mp3agicModule::setter_exampleProp,
		Local<Value>(),
		DEFAULT,
...
// Methods --------------------------------------------------------------------
void Mp3agicModule::getExampleProp(const FunctionCallbackInfo<Value>& args)
{
	...
	if (!methodID) {
		methodID = env->GetMethodID(Mp3agicModule::javaClass, "getExampleProp", "()Ljava/lang/String;");
		if (!methodID) {
...
void Mp3agicModule::setExampleProp(const FunctionCallbackInfo<Value>& args)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(Mp3agicModule::javaClass, "setExampleProp", "(Ljava/lang/String;)V");
		if (!methodID) {
...
void Mp3agicModule::example(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "example()");
	Isolate* isolate = args.GetIsolate();
	Local<Context> context = isolate->GetCurrentContext();
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(Mp3agicModule::javaClass, "example", "()Ljava/lang/String;");
		if (!methodID) {
...
void Mp3agicModule::getter_exampleProp(Local<Name> property, const PropertyCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
...
void Mp3agicModule::setter_exampleProp(Local<Name> property, Local<Value> value, const PropertyCallbackInfo<void>& args)
{
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
