...
#define TAG "de.appwerft.mp3agic"

using namespace v8;

static Persistent<Object> bindingCache;

static void Mp3agic_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Mp3agic.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::Mp3agicBindings::lookupGeneratedInit(
...
static void Mp3agic_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Mp3agic_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Mp3agic_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::Mp3agicBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry Mp3agicBinding = {
	"de.appwerft.mp3agic",
	Mp3agic_init,
	Mp3agic_dispose
};
...
Java_de_appwerft_mp3agic_Mp3agicBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.mp3agic", &Mp3agicBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::Mp3agicBindings::lookupGeneratedInit));
}
